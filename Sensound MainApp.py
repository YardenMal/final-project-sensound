...
class NewApp(MDApp):
    ...
    def finally_selected_converted(self, selection):
        if selection:
            ...
            if self.wavFile:
                ...
                select = NewApp.selectedRecordAnalyzing(self, "clear_recording.wav")
                return select
    ...
    def insertFeelingsToDB(self):
        ...
        mycursor = conn.cursor()
        nowDate = date.today()
        print("insert feeling")
        self.formatted_date = nowDate.strftime("%d-%m-%Y")

        sqlInsert = "INSERT INTO sensound.UsersEmotions (DateOfRecord, email, sad, happy, angry, fearful, neutral) VALUES (%s, %s, %s, %s, %s, %s,%s)"
        val = (self.formatted_date, self.email, self.sad, self.happy, self.angry, self.fearful, self.neutral)
        print(self.formatted_date, self.email, float(self.sad), float(self.happy), float(self.angry), float(self.fearful), float(self.neutral))
        mycursor.execute(sqlInsert, val)
        conn.commit()
